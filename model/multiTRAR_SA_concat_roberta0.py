...
class multiTRAR_SA_concat_roberta0(torch.nn.Module):
  # define model elements
    def __init__(self,bertl_text,vit, opt):
        super(multiTRAR_SA_concat_roberta0, self).__init__()
        ...
        self.trar = model.TRAR.multiTRAR_SA_model(opt)
    ...
    def forward(self, input):
        ...
        out = self.classifier(torch.concat([out1, bert_text[:, 0, :]], dim=1))
...
def build_multiTRAR_SA_concat_roberta0(opt,requirements):
    ...
    return multiTRAR_SA_concat_roberta0(bertl_text,vit,opt)
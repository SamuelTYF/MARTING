...
class Co_attentionVariant(torch.nn.Module):
    def __init__(self, len):
        ...
        self.max_pool = torch.nn.AvgPool2d(kernel_size = (len,1))

    def forward(self, H, T, V):
        batch=H.shape[0]
        T_tmp = T.clone()
        T = T.permute(0, 2, 1)
        ...
        C = self.tanh(z_2)
        alpha = self.max_pool(C)*V.unsqueeze(1)
        HT = torch.matmul(alpha, T_tmp).reshape(batch, 768)
        return HT

class Roberta_CoAttentionVariant(torch.nn.Module):
    def __init__(self,bertl_text,bertl_attribute, opt):
        super(Roberta_CoAttentionVariant, self).__init__()
        ...
        if "len" not in opt:
            opt["len"]=360
        self.co_attention = Co_attentionVariant(opt["len"])
    ...
    def forward(self, input):
        ...
        out2 = self.co_attention(bert_text, bert_attribute,input[self.input3])
...
def build_Roberta_CoAttentionVariant(opt,requirements):
    ...
    return Roberta_CoAttentionVariant(bertl_text,bertl_attribute,opt)
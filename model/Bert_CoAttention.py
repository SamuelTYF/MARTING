...
class Bert_CoAttention(torch.nn.Module):
    def __init__(self,bertl_text,bertl_attribute, opt):
        super(Bert_CoAttention, self).__init__()
    ...
    def forward(self, input):
        ...
        bert_embed_text = self.bertl_text.embeddings(input[self.input1])

        bert_embed_attribute = self.bertl_attribute.embeddings(input[self.input2])
...
def build_Bert_CoAttention(opt,requirements):
    from transformers import BertModel
    bertl_text = BertModel.from_pretrained('bert-base-uncased')
    bertl_attribute = BertModel.from_pretrained('bert-base-uncased')
    return Bert_CoAttention(bertl_text,bertl_attribute,opt)
...
class Clip(torch.nn.Module):
    def __init__(self,clip,opt):
        ...
        assert("len" in opt)
        self.len=opt["len"]
        self.fc = torch.nn.Linear(self.len+self.len+self.len,2)
    ...
    def forward(self,input):
        ...
        # print(input[self.input1].shape,input[self.input2].shape)
        text_features = self.clip.encode_text(input[self.input1])
        image_features = self.clip.encode_image(input[self.input2])

        out = torch.cat((text_features, image_features, self.co_attention(image_features.unsqueeze(1),text_features.unsqueeze(1))), dim = 1)
        out = self.fc(out)
        result = self.sigm(out)

        del text_features, image_features, out

        return result

def build_Clip(opt,requirements):
    assert("clip" in requirements)
    return Clip(requirements["clip"],opt)
...
class TRAR_Twostream_add(torch.nn.Module):
  # define model elements
    def __init__(self,bertl_text,vit, opt):
        super(TRAR_Twostream_add, self).__init__()
        ...
        self.trar_text = model.TRAR.TRAR(opt)
        opt["backbone"] = "img_guide"
        self.trar_img = model.TRAR.TRAR(opt)
        self.sigm = torch.nn.Sigmoid()
        self.proj_norm = LayerNorm(opt["output_size"])
    ...
    def forward(self, input):
        ...
        out2 = self.trar_img(img_feat, bert_embed_text,input[self.input3].unsqueeze(1).unsqueeze(2))
        proj_feat = out1 + out2
...
def build_TRAR_Twostream_add(opt,requirements):
    ...
    return TRAR_Twostream_add(bertl_text,vit,opt)